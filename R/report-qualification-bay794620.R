#' @title report-qualification-bay794620
#' @description
#' Qualification Report for `Open-Systems-Pharmacology/BAY794620-Model` repository
...
qualificationProject <- "BAY794620-Model"
snapshotFile <- "BAY794620.json"
...
download.file("https://github.com/Open-Systems-Pharmacology/BAY794620-Model/archive/refs/heads/master.zip", destfile = "archive.zip")
unzip("archive.zip", exdir = "archive")
unlink("archive.zip")
dir.create(workingDirectory)
file.copy("archive/BAY794620-Model-master/Evaluation/Input", workingDirectory, recursive = TRUE)
# In this repo, the snapshot is outside the evaluation folder and its path needs to be updated
file.copy(file.path("archive/BAY794620-Model-master/", snapshotFile), workingDirectory, recursive = TRUE)
