#' @title report-qualification-ugt-editor
#' @description
...
qualificationProject <- "Qualification_Plan_Editor"
...
file.copy("archive/Pediatric_Qualification_Package_UGT1A1_Ontogeny-master/Qualification/Input", workingDirectory, recursive = TRUE)
unlink("archive", recursive = TRUE)

#' @description Code hereafter is adapted from `createQualificationReport()` template
qualificationPlanName <- "Qualification-Ontogeny-UGT1A1.json"
qualificationPlanFile <- file.path(workingDirectory, "input", qualificationPlanName)


#' @description 
#' Convert qualification plan to Excel, then back to json
#' using `ospsuite.qualificationplaneditor` package
excelQualificationPlan <- gsub(pattern = "json", replacement = "xlsx", x = qualificationPlanFile)
ospsuite.qualificationplaneditor::toExcelEditor(
  fileName = excelQualificationPlan,
  qualificationPlan = qualificationPlanFile
)
ospsuite.qualificationplaneditor::excelToQualificationPlan(
  excelFile = excelQualificationPlan, 
  qualificationPlan = qualificationPlanFile
)
