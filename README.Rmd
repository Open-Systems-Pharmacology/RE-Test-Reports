...
testResults <- testResultsRaw %>%
  mutate(
    ...
    Duration = real,
    N = nb,
    Success = if_else(nb==0, "", paste0(
      "![](https://geps.dev/progress/",
      round(100 * passed / nb, 0),
      ")"
    )),
    Warning = if_else(nb==0, "", paste0(round(100 * warning / nb, 0), "%")),
    Failed = if_else(nb==0, "", paste0(round(100 * failed / nb, 0), "%"))
  ) %>%
...
# Map report to script Naming convention for scripts was report lower case of directory
availableScripts <- unname(sapply(
  availableReports,
  function(reportDir){
    for(rScript in list.files("./R", pattern = ".R")){
      scriptContent <- readLines(file.path("R", rScript), warn = FALSE)
      reportDefinition <- scriptContent[grepl(pattern = "reportFolder <-", scriptContent)]
      if(grepl(pattern = reportDir, reportDefinition)){
        return(rScript)
      }
      next
    }
